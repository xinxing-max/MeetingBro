"""Verify weak-speech rescue emits text when pre-VAD misses quiet audio.

The synthetic source emits low-RMS chunks.  Pre-VAD is configured with a high
energy fallback threshold so it should not produce normal VAD chunks; the rescue
buffer should still run one ASR attempt and emit a segment.
"""
...
class WeakAudioSource:
    ...
    async def stream(self):
        for i in range(4):
            samples = np.ones(16_000, dtype=np.float32) * 0.006
            yield AudioChunk(samples=samples, sample_rate=16_000, start_time=float(i))
    ...
    def transcribe(
        ...
        *,
        ...
        return [
            ASRSegment(
                start_time=0.0,
                end_time=min(1.0, len(samples) / sample_rate),
...
async def main():
    ...
    cfg = SessionConfig(
        ...
        weak_speech_rescue_window_seconds=2.0,
    ...
    print("PASS: weak-speech rescue recovered quiet audio missed by pre-VAD.")
