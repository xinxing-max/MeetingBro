...
async def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        storage = Storage(Path(tmp) / "verify_pending_silence_commit.db")
        try:
            asr = _TailOnlyASR()
            ...
            if ok:
                print("\nOK: pending tail commits on a following silent window before stop")
                return 0
            print("\nFAIL: pending tail was not committed on silence before stop")
            return 1
        finally:
            storage.close()
